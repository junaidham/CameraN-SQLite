...
//
    ...
    // Adding new contact
    public long addContact(ContactPhoto contact) {
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(COLUMNS_NAME, contact.getName()); // Contact Name
        values.put(COLUMNS_PHOTO, contact.getPhoto()); // Contact Phone Number
...
//    }
    ...
    public List<ContactPhoto> getAllContacts() {
        List<ContactPhoto> contactList = new ArrayList<ContactPhoto>();
        // Select All Query
       // String selectQuery = "SELECT  * FROM "+ TABLE_NAME + "ORDER BY"+COLUMNS_ID;  //no such table: img_tableORDER
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                contactPhoto.setPhoto(cursor.getBlob(cursor.getColumnIndex(COLUMNS_PHOTO)));
    ...
    public void deleteContact(ContactPhoto contact) {
        SQLiteDatabase db = this.getWritableDatabase();
